...
function showFlash({ message, type }: { message: string; type: 'success' | 'error' }) {
...
const { data: items, refresh } = await useAsyncData(props.resource, () => apiFetch(`/${props.resource}`))

const fields = ref<string[]>(props.fields)
...
async function createItem() {
  try {
    ...
    emit('success', { action: 'create' })
  } catch (error) {
    showFlash({ message: "Erreur d'ajout: " + error, type: 'error' })
    emit('error', { action: 'create', error })
  }
}
...
async function updateItem(id: number) {
  try {
    await apiFetch(`/${props.resource}/${id}`, {
      method: 'PUT',
      body: editItem.value,
    })
