  ...
  <div class="flex flex-col gap-2 h-full">
    <div ref="chatContainer" class="grow overflow-y-auto p-2 flex flex-col gap-2">
      <div v-for="msg in messages" :key="msg.id" class="flex flex-col"
           :class="msg.from === currentUserId ? 'items-end' : 'items-start'">
        <div class="max-w-[75%] px-3 py-2 rounded-lg text-sm"
             :class="msg.from === currentUserId ? 'bg-green-100' : 'bg-gray-200'">
          {{ msg.contenu }}
        </div>
